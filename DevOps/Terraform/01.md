...
In this task, you will download and install the Terraform manually.

1. Within the Virtual Machine, open up a new tab on the web browser and navigate to the below URL to install **Terraform**.

```
   https://www.terraform.io/downloads.html
```

2. On the downloads page, scroll down till you find the windows package, and click on **64-bit** to download the terraform zip file.

    ![download](../Infrastructure-as-code-with-Terraform/media/downlaod.png)
...
3. Navigate to the C drive and create a folder named **terraform**, and extract the zip file to the folder.
     ...
     ![terraform](../Infrastructure-as-code-with-Terraform/media/terraform.png)
...
5. To do that search for **Environment variables** from the windows search bar and select it.
    ...
    ![envivariables](../Infrastructure-as-code-with-Terraform/media/envivariables.png)
    
6. On **System properties**, click on **Environment Variables**.

    ![system](../Infrastructure-as-code-with-Terraform/media/system.png)
    
7. Now under system variables, select **Path (1)** and click on **Edit (2)**.

    ![edit](../Infrastructure-as-code-with-Terraform/media/edit.png)
    
8. Selct **New(1)** then add the path **C:\terraform** **(2)** and click on **Ok (3)** to save it.

    ![new](../Infrastructure-as-code-with-Terraform/media/new.png)
...
You can have a look at sample configuration of Terraform file `main.tf` below:
...
In this task, you will learn about how to write and Initialize the Terraform Configuration.

1. In your LabVm, right-click on the **Windows icon (1)** from the bottom left corner and select **Windows Powershell (Admin) (2)**.

    ![poweshell](../Infrastructure-as-code-with-Terraform/media/powershell.png)
...
1. In the PowerShell terminal, run the following command to install Azure CLI.
   ...
   ```
     ![Azure cli](../Infrastructure-as-code-with-Terraform/media/Azure%20cli.png)
     
1. After installing Azure CLI, close the Windows Powershell and re-open it again.
...
3. Onceyou are authenticated to Azure, you will be presented with subscription information as result.

   ![Cli](../Infrastructure-as-code-with-Terraform/media/cli.png)
...
1. Now Copy and paste the below command in the PowerShell terminal to create a new folder called `learn-terraform-azure` and minimize the terminal.
     ...
     ```
7. Copy and paste the below configuration in a new file called `main.tf`, and save it under **C:\learn-terraform-azure**
...
1. Once you have initiated the terraform, you will be presented with the below message.
    ...
    ![init](../Infrastructure-as-code-with-Terraform/media/init.png)
...
1. Run the below command to automatically update configurations in the current directory for readability and consistency.
     ...
     ```
       terraform fmt
     ```
 1. Terraform will print out the names of the files it modified if any. In this case, your configuration file was already formatted correctly, so Terraform won't return any file names.

     ![fmt](../Infrastructure-as-code-with-Terraform/media/fmt.png)
...
1. You can also make sure your configuration is syntactically valid and internally consistent by running the below command.
    ...
    ![validate](../Infrastructure-as-code-with-Terraform/media/validate.png)
    
1. Run the below command to apply your configuration.

     ```
       terraform apply
     ```

    ![apply](../Infrastructure-as-code-with-Terraform/media/apply.png)
    
     > **Note**: This output shows the execution plan and will prompt you for approval before proceeding. If anything in the plan seems incorrect or dangerous, it is safe to abort here with no changes made to your infrastructure. Type yes at the confirmation prompt to proceed.

1. When you apply your configuration, Terraform writes data into a file called **terraform.tfstate**. This file contains the IDs and properties of the resources Terraform created so that it can manage or destroy those resources going forward. 

1. You can inspect the current state using the following command.

   ```
      terraform show
   ```
   
     ![show](../Infrastructure-as-code-with-Terraform/media/show.png)
     
1. To review the information in your state file, use the **state** command. If you have a long state file, you can see a list of the resources you created with Terraform by using the **list** subcommand.

     ```
        terraform state list
     ```
       
      ![list](../Infrastructure-as-code-with-Terraform/media/list.png)
      
1. Use the following command, to see a full list of available commands to view and manipulate the configuration's state.

   ```
      terraform state
   ```
   ![state](../Infrastructure-as-code-with-Terraform/media/state.png)

1. Now you have successfully created a **Resource group** under **Westus2** region.
...
In this exercise, you have covered the following:
 ...
 - Created Resource group using Terraform

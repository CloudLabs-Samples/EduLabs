...
A Kubernetes service is a logical abstraction for a deployed group of pods in a cluster that all perform the same function.
...
1. Run the below command to store the value of the node port inside an environment variable. We are creating an environment variable called **NODE_PORT** that has the value of the Node port assigned.
